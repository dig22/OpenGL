...
unsigned int Shader::CompileShader(unsigned int type, const std::string &source)
{
    ...
    const char *src = source.c_str(); //pointer to string start (raw string)
    ...
    if (result == GL_FALSE)
    {
        ...
        std::cout << "Error! Falied to compile" << (type == GL_VERTEX_SHADER ? "vertex" : "fragment")
                  << " shader" << std::endl;
...
void Shader::setUnifoirmMat4f(const std::string &name, const glm::mat4 &matrix)
{
